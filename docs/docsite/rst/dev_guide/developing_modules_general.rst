...
Note that this example requires Docker to be installed and running. If you prefer not to use a
container for this, you can choose to use ``--venv`` instead of ``--docker``. Or you can use the Python testing wrapper by passing ``--tox`` instead of ``--docker``.
