...
Your collection name should represent the *product* or *OS* name, not the company name. Each module should have the same (or similar) prefix.
...
In collections or in the ansible/ansible repo, modules need maintenance. Follow these guidelines when submitting bug fixes, new features, or other changes to collections hosted in the `ansible-collections <https://github.com/ansible-collections>`_ namespace on GitHub:

``TODO: update directions on botmeta and docs; research whether other info belongs here!``
...
If the module documentation hasn't been published live yet, please let a member of the Ansible Core Team know in the ``#ansible-docs`` IRC channel.
