...
ENOS は、Enable モード (権限昇格) に対応します。ここでは、Ansible の ENOS で Enalbe モードを使用する方法を説明します。 
...
レガシー Playbook の場合でも、ENOS は ``ansible_connection: local`` に対応します。できるだけ早期に ``ansible_connection: network_cli`` を使用するモダナイゼーションが推奨されます。

Ansible での CLI の使用
================================================================================

CLI の例: ``group_vars/enos.yml``
--------------------------------------------------------------------------------
