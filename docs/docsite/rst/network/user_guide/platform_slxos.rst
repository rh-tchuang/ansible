...
Extreme SLX-OS Ansible モジュールは現時点では CLI 接続のみに対応します。``httpapi`` モジュールは今後追加される可能性があります。
このページには、Ansible で SLX-OS の ``network_cli`` を使用する詳細な方法が記載されています。
